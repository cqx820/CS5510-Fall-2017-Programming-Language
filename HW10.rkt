...
(module+ test
  ...
  (test (run-prog '1)
        '1)
  ;;(test/exn (run-prog '{if0 {lambda {[x : num]} x} 1 2}) "not a number")
  ...
  (test (run-prog `empty)
        `list)
  (test (run-prog '{lambda {[x : num]} {* x 2}}) `function)
  ...
  (type-case ExprC a
    ...
    [restC (a) (type-case Value (interp a env)
                 [listV (elems) (if (empty? elems)
                                    ...
                                    (listV (rest elems)))]
                 [else (error 'interp "not a list")])]
    [if0C (tst thn els) ;;(type-case Value (interp tst env)
          ;; [numV (n) (if (= n 0)
          ;;            (interp thn env)
          ;;            (interp els env))]
...
(define (typecheck [a : ExprC] [tenv : TypeEnv])
  (type-case ExprC a
    ...
    [consC (l r)
           ;;(listofT (boolT))
           (local [(define t (varT (box (none))))
                   (define l-type (typecheck l tenv))
                   (define r-type (typecheck r tenv))]
             (begin
               (unify! t l-type l)
               (unify! r-type (listofT l-type) r)
               r-type))]
    ...
    [if0C (tst thn els)
          (let ([t (varT (box (none)))])
            (begin
              ...
              (unify! t (typecheck thn tenv) thn)
              (unify! t (typecheck els tenv) els)
              t))]))

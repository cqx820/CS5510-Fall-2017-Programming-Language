...
(module+ test
  ...
  (test (interp-expr (parse '{snd {pair 1 2}}))
        '2)
  (test/exn (interp-expr (parse '{fst {+ 1 7}})) "not a pair")
  (test/exn (interp-expr (parse '{snd {+ 1 7}})) "not a pair")
  (test (interp-expr (parse '{lambda {x} {* x 5}}))
        `function)
...
(define (interp [a : ExprC] [env : Env]) : Value
  (type-case ExprC a
    ...
    [if0C (f l r)
          (if (equal? (numV 0) (interp f env))
              (interp l env)
              (interp r env))]
    [pairC (l r) (pairV l r env)]
    [fstC (a) (type-case Value (interp a env)
                [pairV (l r env-p) (interp l env-p)]
                [else (error 'interp "not a pair")])]
    [sndC (a) (type-case Value (interp a env)
                [pairV (l r env-p) (interp r env-p)]
                [else (error 'interp "not a pair")])]))
(define (interp-expr e)
  (let ([v-expr (interp e mt-env)])

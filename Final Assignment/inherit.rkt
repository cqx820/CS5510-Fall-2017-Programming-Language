...
(define-type ExprI
  ...
  [setI (obj-expr : ExprI)
        (field-name : symbol)
        (arg-expr : ExprI)] ;;Added for Q5
  ;;Q7
  [newarrayI (type : Type)
             (size : ExprI)
             (init-val : ExprI)]
  [arrayrefI (arr-of : ExprI)
             (index : ExprI)]
  [arraysetI (arr-of : ExprI)
             (index : ExprI)
             (new-val : ExprI)]
  ...
  (local [(define (recur expr)
    ...
    (type-case ExprI a
      ...
      [setI (expr field-name arg)
            (setC (recur expr) field-name (recur arg))] ;;set for Q5
      ;;Q7
      [newarrayI (type size init-val)
                 (newarrayC type (recur size) (recur init-val))]
      [arrayrefI (arrof index)
                 (arrayrefC (recur arrof) (recur index))]
      [arraysetI (arrof index new-val)
                 (arraysetC (recur arrof) (recur index) (recur new-val))]
...
(module+ test
  (test (expr-i->c (nullI) 'object) (nullC));;add test for null for Q4
  
  (test (expr-i->c (setI (nullI) 'x (numI 2)) 'object) ;;add tests for set for Q5
        (setC (nullC) 'x (numC 2)))
  (test (expr-i->c (setI (numI 3) 'x (numI 2)) 'object)
        (setC (numC 3) 'x (numC 2)))
  ...
  (test (expr-i->c (arraysetI (numI 3) (numI 1) (numI 2)) 'object)
        (arraysetC (numC 3) (numC 1) (numC 2)))
  ...
  (type-case ClassC c
    [classC (name s-name field-names methods)
            (type-case ClassC (flatten-super name classes i-classes)
              [classC (super-name s-super-name super-field-names super-methods)
                      (classC
                       name
                       super-name
...
(module+ test
  (test (interp-i (numI 0) empty)
        (numV 0))
  (test (interp-i (nullI) empty) (nullV)) ;;test for null for Q4
  ...
  (test/exn (interp-i
             ...
             (list posn-i-class
                   posn3d-i-class))
            "null reference")) ;;test for null for Q4

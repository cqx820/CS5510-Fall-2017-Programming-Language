...
(define (compile a-fae env)
  (type-case ExprC a-fae
    ...
    [boxC (arg) (code-malloc1 19 (compile arg env))]
    [unboxC (arg) (code-malloc1 20 (compile arg env))]))
  ...
  (case (code-ref expr-reg 0)
    ...
    [(19) ; box
     (begin
       ...
       ;;(set! k-reg (malloc1 22 (code-ref k-reg 2)))
       (set! expr-reg (code-ref expr-reg 1))
       (interp))]
    [(20) ; unbox
     (begin
       (set! k-reg (malloc1 23 k-reg))
       ;;(set! k-reg (malloc1 23 (code-ref k-reg 2)))
       (set! expr-reg (code-ref expr-reg 1))
       (interp))]))
  ...
  (case (ref k-reg 0)
    ...
    [(22) ; boxK
     (begin
       ...
       ;;(set! k-reg (malloc1 21 (code-ref k-reg 1)))
       (set! k-reg (ref k-reg 1))
       (continue))]
    [(23) ; unboxK
     (begin
       (set! v-reg (ref v-reg 1))
       ;;(set! k-reg (code-ref k-reg 1))
       (set! k-reg (ref k-reg 1))
       (continue))]))
